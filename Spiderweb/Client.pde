...
/*
  SClient - basic network client implementation
 ...
 * @instanceName client any variable of type SClient
 * @usage Application
 */
public class SClient implements Runnable {
  ...
  public SClient(PApplet parent, String host, int port) {
    ...
    try {
      ...
      // public void clientEvent(processing.net.SClient)
  ...
  /**
   * Disconnects from the server. Use to shut the connection when you're finished with the SClient.
   * =advanced
   * Disconnect from the server and calls disconnectEvent(SClient c)
   * in the host PApplet.
   * <P/>
  ...
  public void stop() {
    ...
    if (disconnectEventMethod != null) {
      try {
        disconnectEventMethod.invoke(parent, new Object[] { 
          this
        }
        );
      } 
    ...
    while (Thread.currentThread() == thread) {
      try {
        ...
        if (clientEventMethod != null) {
          try {
            clientEventMethod.invoke(parent, new Object[] { 
              this
            }
            );
          } 
  ...
  /**
   * Returns the IP address of the computer to which the SClient is attached.
  ...
  //parent.die("Error inside SClient." + where + "()", e);
